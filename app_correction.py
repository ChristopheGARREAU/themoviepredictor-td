#!/usr/bin/python                           # shebang : faire comprendre au système avec quel interprêteur il peut lire  le fichier : débute par #!
# -*- coding: utf-8 -*-                     # permet à l'éditeur de texte de définir l'encodage du fichier
...
def insert_people_query(first_name, last_name):
...
def insert_people(first_name, last_name):
    cnx = connectToDatabase()
    cursor = createCursor(cnx)
    results = cursor.execute(insert_people_query(first_name, last_name))
    cnx.commit()
...
if args.context == "people":
    ...
    if args.action == "insert":
        ...
        insert_people(first_name=args.firstname, last_name=args.lastname)
